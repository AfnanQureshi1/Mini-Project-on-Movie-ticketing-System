...
<head>	
	...
	<style type="text/css">
		...
		.bg { 
			/* The image used */
			background-image: url("image/addMovieBackground.jpg");
		...
		<div class="container">  
			<form id="contact" action="addMovie.php" method="post" enctype="multipart/form-data">
				...
				<input style="padding: 10px;" type="file" name="image" required autofocus>
                                ...
                                <input style="padding: 10px;" type="file" name="video" required autofocus>
			...
			<div class="wrapper">
				<button class="btn-default" onclick="document.location.href='admin.php'" > <span class='glyphicon glyphicon-chevron-left'> </span>BACK TO THE ADMIN PAGE</button>
			</div>
...
$target="uploadimages/".basename($_FILES['video']['name']);
		$image=$_FILES['video']['name'];
		$image_tmp=$_FILES['video']['tmp_name'];



			if(move_uploaded_file($video_tmp, $target))
			{
